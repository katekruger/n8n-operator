...
| Every v1 tool's v2 form (`environment` argument, result fields, pagination, authorization filtering, new errors) | AC-37, AC-38, AC-39, AC-44 | `tests/contract/test_mcp_tool_inventory.py` (existing, still passes: 12 tools unchanged in v1); authorization filtering: `tests/integration/test_authorization_service.py`, `tests/property/test_no_enumeration.py`; `environment` argument itself: still planned, stage 04 | [MCP_TOOLS.md](MCP_TOOLS.md) §5.9 | 02–04 | partial — authorization filtering done (stage 03); `environment` argument/pagination-by-environment/new environment errors remain stage 04 |
...
| RBAC role-capability matrix, workflow×environment intersection | AC-38, AC-39 (partial — see below) | `tests/property/test_rbac_matrix.py`, `tests/property/test_no_enumeration.py` (monotonicity), `tests/integration/test_authorization_service.py` | [ADR-015](adr/ADR-015-rbac-authorization-evaluation.md) | 03 | done (workflow-scope + role-capability); environment-scope conjunct is implemented and property-tested but not reachable from a real v1 tool call until stage 04 adds the `environment` argument — see THREAT_MODEL.md RR-13 |
