...
```bash
gh release delete v1.0.0rc2 --cleanup-tag
```
...
```bash
git push origin :refs/tags/v1.0.0rc2
git tag -d v1.0.0rc2
```

Never force-push over or reuse a tag name that was ever public — cut a new, higher
version instead (e.g. `v1.0.0rc3`), even for a same-day fix. A reused tag pointing at
...
Yanking does not delete the file — `pip install n8n-operator==1.0.0rc2` still works if
