...
(phase 5); ``serve approval``, ``operations``, ``audit`` arrive with the phases that
implement them (BUILD_PLAN section 12). ``app`` is the object ``pyproject.toml``'s
``[project.scripts]`` entry point and ``__main__.py`` both invoke — there is exactly one
Typer application, regardless of entry point.
